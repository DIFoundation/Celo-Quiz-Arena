...
export default function HostLobby() {
  ...
  // Load quiz data and participants
  useEffect(() => {
    loadQuizData();
  }, [loadQuizData, quizId]);
  ...
  useEffect(() => {
    ...
    socket.on("player_joined", (data: { wallet: string; name?: string; id?: number }) => {
      ...
      loadQuizData(); // Reload to get updated participant list
    });
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-6 mb-8">
        ...
        {participants.length === 0 ? (
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="text-sm">Share Quiz ID <span className="font-mono font-bold">{quizId}</span> with players</p>
          </div>
        ) : (
      ...
      <div className="bg-linear-to-r from-green-50 to-emerald-50 dark:from-slate-800 dark:to-slate-900 rounded-lg shadow-lg p-6">
        ...
        {participants.length === 0 && (
          <p className="text-sm text-gray-500 text-center mt-3">
            At least one player must join to start
          </p>
        )}
      </div>
    </div>
